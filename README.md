...
Bootstrapped from [andenacitelli/credit-card-bonuses-api](https://github.com/andenacitelli/credit-card-bonuses-api) to get real issuer and card identity data (168 cards, 16 issuers) without hand-typing every name. That source only has sign-up bonus data, not recurring benefit terms, so **`benefits: []` and `earningRates: []` are still empty on almost every card** — only `chase-sapphire-preferred` and `american-express-platinum` have been filled in and verified against official sources so far, as a reference example for filling in the rest.
