...
- Within `bonusCategoryDetail`, use `eligibleCategories` **or** `categorySelection`, never both (the schema enforces this):
  - `eligibleCategories` — every listed category earns the multiplier simultaneously.
  - `categorySelection` — the multiplier applies to a subset picked from a menu. Set `mode` to `user_choice` (cardholder selects — BoA Customized Cash), `issuer_rotating` (issuer announces per cycle — Freedom Flex; pair with `requiresActivation` and `anchor: "fixed_dates"`), or `automatic_top_category` (issuer applies it to the user's highest-spend category — Citi Custom Cash). The menu goes in `options`; set `selectCount` when more than one applies at once.
  - **`issuer_rotating` requires `currentCycle`** (e.g. `"2026-Q3"`), because `options` there is only the list for one announced quarter and is wrong the moment the next one begins. When you refresh a rotating card during the daily pass, update `options`, `currentCycle`, and `renewalPeriod.fixedDates` together — a stale `options` with a fresh `lastVerified` is worse than no data, since it reads as confidently current.
  - This distinction isn't cosmetic: recording a six-category menu as `eligibleCategories` tells every consumer the card earns the bonus on all six at once, which produces confidently wrong "use this card" recommendations.
