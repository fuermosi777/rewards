...
- Within `bonusCategoryDetail`, use `eligibleCategories` **or** `categorySelection`, never both (the schema enforces this):
  ...
  - `categorySelection` — the multiplier applies to a subset picked from a menu. Set `mode` to `user_choice` (cardholder selects — BoA Customized Cash), `issuer_rotating` (issuer announces per cycle — Freedom Flex; pair with `requiresActivation` and `anchor: "fixed_dates"`), or `automatic_top_category` (issuer applies it to the user's highest-spend category — Citi Custom Cash). The menu goes in `options`; set `selectCount` when more than one applies at once (US Bank Cash+ picks two).
